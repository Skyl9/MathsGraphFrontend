...
# Si VITE_BACKEND_LINK n'est pas défini, logguer un avertissement clair
if [ -z "$VITE_BACKEND_LINK" ]; then
  echo "[env.sh] ATTENTION : VITE_BACKEND_LINK n'est pas défini ! Les appels API échoueront."
fi

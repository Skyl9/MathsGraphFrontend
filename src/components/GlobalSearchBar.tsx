...
import { Autocomplete, TextField, CircularProgress, InputAdornment, Box, Typography, Chip, useTheme } from '@mui/material';
...
export const GlobalSearchBar: React.FC = () => {
    ...
    const handleSelect = (_event: React.SyntheticEvent, newValue: SearchResult | string | null) => {
        if (typeof newValue === 'object' && newValue !== null) {
            ...
            if (newValue.entity_type === 'mathematicien') navigate(`/mathematicien/${newValue.id}`);
            if (newValue.entity_type === 'category') navigate(`/category/${newValue.id}`);
            setOpen(false);
        }
    };
    ...
    return (
        <Autocomplete
            sx={{ 
                ...
                '&.Mui-focused': {
                    bgcolor: theme.palette.mode === 'dark' ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.01)',
                ...
                return (
                    <li {...restProps} key={`${option.entity_type}-${option.id}`}>
                        <Box display="flex" justifyContent="space-between" alignItems="center" width="100%" sx={{ py: 0.5 }}>
                            <Typography variant="body2" sx={{ fontWeight: 550 }}>{option.nom}</Typography>
                            <Chip 
                                size="small" 
                                label={
                                    option.entity_type === 'concept' ? 'Concept' :
                                    option.entity_type === 'mathematicien' ? 'Mathématicien' : 'Catégorie'
                                }
                                color={
                                    option.entity_type === 'concept' ? 'info' :
                                    option.entity_type === 'mathematicien' ? 'secondary' : 'warning'
                                }
                                sx={{ 
                                    fontWeight: 700, 
                                    fontSize: '0.65rem',

...
export const HistoryModal: React.FC<HistoryModalProps> = ({
  ...
  const [selectedRollbackFilter, setSelectedRollbackFilter] =
    useState<RollbackFilter>("Tous types");
  ...
  const versions: VersionGroup[] = useMemo(() => {
    ...
    return Array.from(map.values()).sort(
      (a, b) => a.globalVersion - b.globalVersion,
    );
  }, [history]);
  ...
  const categories = useMemo(
    () => [
      "Toutes",
      ...Array.from(new Set(history.map((h) => h.field_modified))),
    ],
    [history],
  );
  ...
  }, [versions, selectedCategory, selectedRollbackFilter]);

  const [prevFilteredLength, setPrevFilteredLength] = useState(0);

  if (filteredVersions.length !== prevFilteredLength) {
    setPrevFilteredLength(filteredVersions.length);
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg">
          ...
          <>
            ...
            {current.entries
              .filter(
                (e) =>
                  selectedCategory === "Toutes" ||
                  e.field_modified === selectedCategory,
              )
              .map((entry, i) => {
                return (
      ...
      {filteredVersions.length > 0 && (
        <DialogActions>
          ...
          <Button
            onClick={() =>
              setCatIdx((idx) => Math.min(filteredVersions.length - 1, idx + 1))
            }

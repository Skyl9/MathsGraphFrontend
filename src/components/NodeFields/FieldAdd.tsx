...
const FieldAdd: React.FC<FieldWithAddProps> = ({ label, onChange,createField }) => {
    ...
    return (
        <Box mt={2}>
            {!adding ? (
                <Button sx={{ mt: 1 }} onClick={() => setAdding(true)}>Ajouter un {label.toLowerCase()}</Button>
            ) : (
                <Box display="flex" gap={1} mt={1}>
                    <TextField size="small" value={newItem} onChange={(e) => setNewItem(e.target.value)} label={`Nouveau ${label.toLowerCase()}`} />

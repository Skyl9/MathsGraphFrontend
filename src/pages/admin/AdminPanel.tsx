...
export default function AdminPanel() {
  ...
  const handleRecalculate = async () => {
    ...
    try {
      ...
      alert(
        "Recalcul terminé ! Les nouvelles positions seront visibles après un rechargement.",
      );
  ...
  const {
    ...
    isLoading: loading,
    error: queryError,
  } = useQuery<AllNodeData[]>({
    queryKey: ["adminNodes"],
    queryFn: async () => {
      const backendLink =
        window.__RUNTIME_CONFIG__?.VITE_BACKEND_LINK ||
  ...
  const updateNodeMutation = useMutation({
    mutationFn: async ({
      ...
      id: number;
      rowToUpdate: Partial<AllNodeData>;
    }) => {
      const backendLink =
        window.__RUNTIME_CONFIG__?.VITE_BACKEND_LINK ||
        import.meta.env.VITE_BACKEND_LINK ||
        "";
      const response = await fetch(backendLink + `/updateNodes/${id}`, {
        method: "PATCH",
  ...
  const getRowHeight = useCallback((params: GridRowHeightParams) => {
    ...
    const contentHeight = params.model.enonce
      ? Math.ceil((params.model.enonce.length / 100) * lineHeight * 16)
      : minHeight;
  ...
  const renderCenteredCell = (params: GridRenderCellParams) => (
    <div className="cell-wrapper">{params.value as string}</div>
  );
    ...
    () => [
        ...
        renderCell: (params: GridRenderCellParams<AllNodeData>) => {
          ...
          const categoryName =
            typeof val === "object" && val !== null && "category" in val
              ? (val as { category: string }).category
              : String(val || "");
      ...
      },
      {
        ...
        renderCell: (params: GridRenderCellParams<AllNodeData>) => {
          ...
          return <div key={params.row.id}>{verif ? "✅" : "❌"}</div>;
        },
      },
      {
        field: "sources",
            ...
            <div className="cell-wrapper">
              {sources.length > 0 ? (
                sources.map((source: Source, index: number) => (
                  <div key={index}>{source.titre}</div>
                ))
              ) : (
      ...
      },
      {
        ...
        width: 120,
        renderCell: (params: GridRenderCellParams<AllNodeData>) => (
          <button onClick={() => saveChanges(params.row.id)}>
            Sauvegarder
          </button>
        ),
      },
      ...
      <MathJaxContext>
        ...
        <div className="admin-global-actions" style={{ marginBottom: "20px" }}>
          ...
          >
            {isRecalculating
              ? "⏳ Recalcul en cours..."
              : "🔄 Recalculer les positions du graphe"}
          </button>
        </div>
